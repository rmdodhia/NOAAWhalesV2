...
log_file = "Logs/make_spectrograms.log"
...
def generate_stft_spectrogram_batch(segments):
    ...
    spec = torch.abs(spec)
    spec = torch.log1p(spec)  # Non-dB scaling
    return spec.cpu().numpy()

# ---- Spectrogram Extraction per File ----
...
def make_spectrograms_with_labels(sourceAudioFolder, destinationImageFolder, allAnnotationsCsv, sdur=2, overlap=0, species_name=None):
    ...
    logging.info(f"Processing {len(files)} audio files in {sourceAudioFolder}")

    for f in files:
        filepath = os.path.join(sourceAudioFolder, f)
        segs = list(labels[labels['audiofile'] == f]['time_segments'])
        results = extract_spectrograms_from_file(filepath, destinationImageFolder, segs, segment_duration=sdur, overlap=overlap)
    ...
    if species_name and species_name.lower() == 'beluga':
        ...
        scatter_df = rest.sample(frac=0.05, random_state=42)

        df = pd.concat([pos_df, near_df, scatter_df], ignore_index=True)
...
def generate_spectrograms_and_labels_for_species_locations(species_name, location_names, annotation_csv_path=None, sdur=2, overlap=0.4):
    ...
    for name in location_names:
        ...
        src = f"./DataInput/{species_name}/{name}"
        ...
        df = make_spectrograms_with_labels(src, dst, ann, sdur=sdur, overlap=overlap, species_name=species_name)
...
def save_pt_file(args):
    img, output_path = args
    torch.save(torch.tensor(img, dtype=torch.uint8), output_path)
...
def batch_spectrogram_to_images(batch_args):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        list(executor.map(save_pt_file, tasks))
