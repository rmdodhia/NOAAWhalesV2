'''
Combining orca and humpback data with three labels: nothing, orca, humpback
'''
...
for run in runs:
    ...
    train_subset = Subset(SpectrogramDataset(train_image_paths, train_labels, transform), all_train_idx)
    val_subset_from_train = Subset(SpectrogramDataset(train_image_paths, train_labels, transform), val_idx_from_train)
    val_dataset_from_val_location = SpectrogramDataset(val_image_paths, val_labels, transform)
    ...
    test_loader = DataLoader(SpectrogramDataset(test_image_paths, test_labels, transform), batch_size=batch_size, shuffle=False)
