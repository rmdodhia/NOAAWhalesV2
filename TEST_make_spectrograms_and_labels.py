...
def process_wav_file(
    ...
    large_file_samples=10_000_000  
):
    wav, sr = torchaudio.load(wav_path)
    audiofile = os.path.basename(wav_path)
    location = os.path.basename(os.path.dirname(wav_path))

    # --- Decide device and dtype based on file size ---
    if wav.shape[-1] > large_file_samples:
        wave = wav[0].cpu()  # Use CPU for large files
        device = 'cpu'
        dtype = torch.float16
        logging.info(f"Processing {audiofile} on CPU due to large file size: {wav.shape[-1]} samples")
    else:
        if DEVICE == 'cuda':
            wave = wav[0].to(DEVICE).half()  # Use float16 on GPU
            device = DEVICE
            dtype = torch.float16
            logging.info(f"Processing {audiofile} on GPU (float16): {wav.shape[-1]} samples")
        else:
            wave = wav[0].cpu()
            device = 'cpu'
            dtype = torch.float32
            logging.info(f"Processing {audiofile} on CPU: {wav.shape[-1]} samples")
    ...
    # --- For nulls: pick k_null random segments outside all padded regions ---
    null_indices = [i for i in range(num_segments) if i not in padded_indices]
    if len(null_indices) > 0 and k_null > 0:
        rng = np.random.default_rng(seed=123)
        sampled_nulls = rng.choice(null_indices, min(k_null, len(null_indices)), replace=False)
        for i in sampled_nulls:
            seg_start = i * stride
            seg_end = seg_start + segment_duration
            arr = slices[i].numpy()
            fname = f"{location}__{audiofile.replace('.wav','')}__{int(seg_start*1000)}_{int(seg_end*1000)}.pt"
